 ...
 * @param callback 
 */
module.exports.GetBottleInfo = function ( model_id: number, callback: CallableFunction ) {
 ...
 */
module.exports.GetAveragesByMonthYear = function ( model_id: number, callback: CallableFunction ): void {
    ...
    con.connect( function( err: MysqlError ){
        ...
        // Get the bottle info for relevant bottles by distillery and age
        con.query( `SELECT price, auction_month, auction_year FROM bottle_sales WHERE model_id=?;`,
            [ model_id ],
            function( err: MysqlError | null, resultSet ) {
                if ( err ) { throw err; }

                // Create an array from the result set to iterate through
                const results: any = Array.from( resultSet ).reverse();
                var formatted_results: any = [];
                ...
                for ( var key in formatted_results ){
                    ...
                    // Add it to an object
                    final_results[key] = ( ( temp_values.reduce( ( partialSum: any, a: any ) => partialSum + a, 0 ) as number ) / temp_values.length ).toFixed(2);
                }
