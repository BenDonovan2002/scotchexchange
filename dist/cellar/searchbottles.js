 ...
 */
module.exports.GetRelevantBottles_LEGACY = function (distillery, age, abv, vol, rescount, currentPage, callback) {
    var con = mysql.createConnection(mysql_info.getSQL());
    con.connect(function (err) {
        if (err)
            throw err;
        // Get the bottle info for relevant bottles by distillery and age
        con.query(`SELECT * FROM bottle_models WHERE distillery LIKE ? AND age LIKE ? AND bottle_size LIKE ? AND bottled_strength LIKE ?;`, [`%${distillery}%`, `%${age}%`, `%${vol}%`, `%${abv}%`], function (err, resultSet) {
            if (err) {
                throw err;
            }
...
function SearchDatabaseForMatch(searchString, searchProperties) {
    ...
    return new Promise((resolve, reject) => {
        ...
        let c = connection.query(sql, sqlParams, function (err, result) {
            if (err)
                throw err;
...
function RetrieveMatchRanking(searchStringWords, searchResultWords) {
    let count = 0;
    let indexedWords = [];
...
module.exports.GetRelevantBottles = function (searchQuery, rescount, currentPage, callback) {
    SearchDatabaseForMatch(searchQuery, {}).then((results) => {
...
module.exports.GetRelevantBottlesAsync = function (searchQuery, rescount, currentPage) {
    return __awaiter(this, void 0, void 0, function* () {
        let results = yield SearchDatabaseForMatch(searchQuery, {});
        if (results.length != 0) {
            let sortedResultsWithRank = CurateSearchResults(searchQuery, results);
            let sortedResults = sortedResultsWithRank.map((result) => {
                return result[0];
