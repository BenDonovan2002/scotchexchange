...
function UploadBottleModelToDatabase(connection, model) {
    let age = GetBottleAge(model);
    let extraInfo = MergeObjectArray(model);
    let strength = extraInfo.strength == undefined ? "NULL" :
        extraInfo.strength.replace("%", "");
    let size = extraInfo.bottlesize == undefined ? "NULL" :
        ConvertToCl(extraInfo.bottlesize);
    return new Promise((resolve, reject) => {
        connection.query("INSERT INTO bottle_models (pageTitle, pageDescription, age, bottled_strength, bottle_size, image_url, website_url) VALUES (?, ?, ?, ?, ?, ?, ?)", [
            model.title,
            model.description.length > 4096 ? model.description.substring(0, 4096) : model.description,
            age,
            strength,
            size,
            model.image,
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < bottles.length; i++) {
            ...
            console.log(`Uploading bottle model to database (${i}/${bottles.length}): ` + bottle.title);
            let matches = yield SearchDatabaseForMatch(connection, bottle.title);
            if (matches.length == 0) {
                yield UploadBottleModelToDatabase(connection, bottle);
            }
            else {
                console.log("Bottle already exists in database");
            }
        }
        connection.end();
    });
